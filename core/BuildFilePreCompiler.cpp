#include "BuildFilePreCompiler.h"
...
namespace
{
    ...
    void erase(
        ...
        std::vector<std::filesystem::path> const& toErase
    ) {
        for (auto const& n : toErase) {
            auto it = std::find(paths.begin(), paths.end(), n);
...
namespace YAM {
    BuildFilePreCompiler::BuildFilePreCompiler(
    ...
    void BuildFilePreCompiler::findCompilers() {
        throw std::runtime_error("not implemented");
        for (auto const& dep : _buildFile.deps.depBuildFiles) {

        }
    }

    void BuildFilePreCompiler::findGlobs() {
        throw std::runtime_error("not implemented");
        for (auto const& dep : _buildFile.deps.depGlobs) {

        }
    }

    void BuildFilePreCompiler::compileRule(BuildFile::Rule const& rule) {
        std::vector<std::filesystem::path> cmdInputs = compileInputs(rule.cmdInputs);
        std::vector<std::filesystem::path> orderOnlyInputs = compileOrderOnlyInputs(rule.orderOnlyInputs);
        ...
        if (rule.forEach) {
            for (auto const& cmdInput : cmdInputs) {
                std::vector<std::filesystem::path > cmdInputs_({ cmdInput });
    ...
    std::vector<std::filesystem::path> BuildFilePreCompiler::compileInputs(
        BuildFile::Inputs const& inputs
    ) {
        std::vector<std::filesystem::path> inputPaths;
        for (auto const& input : inputs.inputs) {
            std::vector<std::filesystem::path> paths = compileInput(input);
            if (input.exclude) {
                erase(inputPaths, paths);
            } else {
                inputPaths.insert(inputPaths.end(), paths.begin(), paths.end());
            }
        }
        return inputPaths;
    }

    std::vector<std::filesystem::path> BuildFilePreCompiler::compileInput(
        BuildFile::Input const& input
    ) {
        std::vector<std::filesystem::path> inputPaths;
        if (input.isGroup) {
            inputPaths = compileInputGroup(input);
        } else if (Glob::isGlob(input.pathPattern.string())) {
            inputPaths = compileGlob(input.pathPattern);
        } else {
    ...
    std::vector<std::filesystem::path> BuildFilePreCompiler::compileInputGroup(
        BuildFile::Input const& input
    ) {
        throw std::runtime_error("not implemented");
        // inputs must be source files or output files created by already
        // existing commands in this buildfile or in _inputCompilers.
        std::vector<std::filesystem::path> groupContent;
        return groupContent;
    }

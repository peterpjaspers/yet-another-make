...
namespace YAM
{
	...
	public:
        ...
        void getFiles(std::vector<FileNode*>& filesInDir);
        void getSubDirs(std::vector<DirectoryNode*>& subDirsInDir);

        std::map<std::filesystem::path, Node*> const& getContent(); // file + dir nodes
    ...
    private:
        Node* createNode(std::filesystem::directory_entry const& dirEntry);
        ...
        std::map<std::filesystem::path, Node*> _content;
	};
}

...
namespace YAM
{
	...
	public:
		...
		// Perhaps keep this implementation here and override it in 
		// WatchedFileRepository and/or SourceFileRepository. Also because
		// ignore files are likely to be modified. WatchedFileRepository can
		// collect/maintain all ignore info per directory by mirroring all
		// directories in IgnoreNodes that have .ignore files as their 
		// prerequisites. YAM execution than becomes 3 staged:
		//	1) execute ignore mirror
		//  2) execute repo mirrors (directory node has ignore prerequisite)
		//  3) execute build file and command nodes
		//
